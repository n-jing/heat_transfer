...
namespace Jing
{
  ...
  std::tuple<bool, Eigen::Matrix<T, -1, -1>, Eigen::Matrix<U, -1, -1>> ReadTetHex(const char* path)
  {
    ...
    while (!feof(f_in))
    {
      ...
      if (line[0] == 'P' && line[1] == 'O' && line[2] == 'I' &&
          line[3] == 'N' && line[4] == 'T' && line[5] == 'S' && line[6] == ' ')
      {
        ...
        int all_v = 0;
        EXITIF(fscanf(f_in, "%d%d", &cell_num, &all_v) < 1, "CELLS format error");
        int E = (all_v/cell_num) - 1;
        assert(E == 3 || E == 4 || E == 8);
        ...
        for (int t = 0; t < cell_num; ++t)
        {
          ...
          {
            if (std::is_same<U, size_t>::value)
              EXITIF(fscanf(f_in, "%*d%zu%zu%zu%zu", &cells(t, 0), &cells(t, 1),
                            &cells(t, 2), &cells(t, 3)) < 4, "cell format error")
              else
                EXITIF(fscanf(f_in, "%*d%d%d%d%d", &cells(t, 0), &cells(t, 1),
                              &cells(t, 2), &cells(t, 3)) < 4, "cell format error")
          } 
          else if (E == 8)
          {
            if (std::is_same<U, size_t>::value)
              EXITIF(fscanf(f_in, "%*d%zu%zu%zu%zu%zu%zu%zu%zu",
                            &cells(t, 0), &cells(t, 1),
                            &cells(t, 2), &cells(t, 3), &cells(t, 4), &cells(t, 5),
                            &cells(t, 6), &cells(t, 7)) < 8,
                     "CELLS format error")
            else
              EXITIF(fscanf(f_in, "%*d%d%d%d%d%d%d%d%d", &cells(t, 0), &cells(t, 1),
                            &cells(t, 2), &cells(t, 3), &cells(t, 4), &cells(t, 5),
                            &cells(t, 6), &cells(t, 7)) < 8,
                     "CELLS format error");
          ...
          {
            if (std::is_same<U, size_t>::value)
              EXITIF(fscanf(f_in, "%*d%zu%zu%zu", &cells(t, 0), &cells(t, 1),
                            &cells(t, 2)) < 3, "cell format error")
              else
                EXITIF(fscanf(f_in, "%*d%d%d%d", &cells(t, 0), &cells(t, 1),
                              &cells(t, 2)) < 3, "cell format error");
          }
        }
        while (!(line[0] == 'C' && line[1] == 'E' && line[2] == 'L'
  ...
  template<typename T=double, typename U=int>
  std::tuple<bool, Eigen::Matrix<T, -1, -1>, Eigen::Matrix<U, -1, -1>> ReadModel(const char* path)
  {
    std::string str_path(path);
    if (str_path.rfind(".obj") != std::string::npos)
    {
      return ReadObj<T, U>(path);
    }
    else if (str_path.rfind(".vtk") != std::string::npos)
    {
      return ReadTetHex<T, U>(path);
    }
    else
    {
  ...
  template std::tuple<bool, Eigen::Matrix<float, -1, -1>, Eigen::Matrix<int, -1, -1>>
  ReadModel<float, int>(const char* path);
  
  template std::tuple<bool, Eigen::Matrix<double, -1, -1>, Eigen::Matrix<int, -1, -1>>
  ReadModel<double, int>(const char* path);

  template std::tuple<bool, Eigen::Matrix<float, -1, -1>, Eigen::Matrix<size_t, -1, -1>>
  ReadModel<float, size_t>(const char* path);

  template std::tuple<bool, Eigen::Matrix<double, -1, -1>, Eigen::Matrix<size_t, -1, -1>>
  ReadModel<double, size_t>(const char* path);
