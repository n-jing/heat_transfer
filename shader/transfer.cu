...
// settings
  ...
  Eigen::Matrix<unsigned int, -1, -1> cells;
  Eigen::Matrix<FLOAT, -1, -1> verts_norm;
  Eigen::Matrix<unsigned int, -1, -1> cells_norm;
  tie(read_success, verts, verts_norm, cells, cells_norm) = ReadObjWithNorm<FLOAT, unsigned int>(path);
  if (!read_success)
  {
  ...
  Matrix<FLOAT, -1, -1, RowMajor> verts_data = UpdateVertsData<FLOAT, unsigned int>(verts, verts_norm, temperature, cells, cells_norm);
  Matrix<unsigned int, -1, -1, RowMajor> verts_cell = cells;
...
// first, configure the cube's VAO (and VBO)
  ...
  InitBufferEBO<float>(planeVAO, planeVBO, planeEBO, sizeof(plane_vertices), plane_vertices, 8, sizeof(plane_indices), plane_indices, {3, 3, 2}, {0, 3, 6});
